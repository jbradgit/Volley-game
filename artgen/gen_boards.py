"""Generate assets/boards/*.png + boards.json — the L1 perimeter hoarding boards.
...
are FICTIONAL; sold sponsor boards later replace these files via boards.json.

Board size: 396x102 px (= 3x the 132x34 logical slot, spec §3/L1).
NOTE: never use "ads"/"advert" in URLs or filenames — desktop ad-blockers block them.
...
out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "assets", "boards")
...
frame(im, d); save(im, "board_scordagol", 1)
...
frame(im, d); save(im, "board_pitchside", 1)
...
frame(im, d); save(im, "board_kestrel", 2)
...
frame(im, d); save(im, "board_volt", 2)
...
frame(im, d); save(im, "board_netlink", 2)
...
frame(im, d); save(im, "board_portlys", 2)
...
frame(im, d); save(im, "board_megasport", 2)

with open(os.path.join(out_dir, "boards.json"), "w") as fjs:
